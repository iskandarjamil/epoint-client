...
class UpdateCardInformationTest extends TestCase
{
    protected $classname;
    ...
    {
        $this->classname = UpdateCardInformation::class;
        $this->customerData = [
            'first_name' => 'Foo',
            'last_name' => 'Bar',
            'email' => 'foo@bar.com',
    ...
    public function testUpdateUserInfortmation()
    {
        ...
        $this->assertEquals('Card has been successfully updated.', $test->message);
        $this->assertTrue($epoint->isUpdated());

        $card = $epoint->getCard();
        $this->assertEquals($this->customerData['first_name'], $card->first_name);
        $this->assertEquals($this->customerData['last_name'], $card->last_name);
        $this->assertEquals($this->customerData['email'], $card->email);
        $this->assertEquals($this->customerData['phone'], $card->mobile);
        $this->assertEquals($this->customerData['gender'], $card->sex);
        $this->assertEquals($this->customerData['date_of_birth'], $card->dob);
        $this->assertEquals($this->customerData['address_line_1'], $card->address1);
        $this->assertEquals($this->customerData['address_line_2'], $card->address2);
        $this->assertEquals($this->customerData['address_postal_code'], $card->zip);
        $this->assertEquals($this->customerData['address_city'] . ", " . $this->customerData['address_state'], $card->city);
    }
